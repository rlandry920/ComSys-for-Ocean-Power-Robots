...
class CommHandler():
    def __init__(self, window_size=WINDOW_SIZE, reliable_img=True, ordered_delivery=True,
        ...
        # Determines whether images are sent and received using RDT
        self.reliable_img = reliable_img
    ...
    def __read(self):
        if self.comm_mode == CommMode.SATELLITE:
            if(type(self.satellite) is EmailHandler):
                recieved_packets = self.satellite.read_packets()
                for new_packet in recieved_packets:
                    logger.debug(
                        f"Recievied packet from email (ID: {new_packet.id}).")
